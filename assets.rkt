...
(define (lang-lib #:inner [inner #t]
  ...
  (define out
    (new block%
         [label "Language"]
         [label-scale 0.2]
         [label-y-offset -0.205]
         [shape (λ (w h)
                  (filled-ellipse w h #:color "cyan"))]))
  (scale
   (cc-superimpose
    (if outer
        (send out draw 400 400)
        (blank 400 400))
    (if inner
        (send in draw 280 280)
        (blank 280 280)))
   1.5))

(define (part-circle #:producers [p #t]
                     #:filters [f #t]
                     #:playlists [pl #t]
                     #:multitracks [m #t])
  (define ((draw-piece start-angle end-angle color) w h)
    (dc (λ (dc dx dy)
          (define old-brush (send dc get-brush))
          (define old-pen (send dc get-pen))
          (send dc set-brush (new brush% [color color]))
          (send dc draw-arc dx dy w h start-angle end-angle)
  ...
  (define prod
    (new block%
         ...
         [label-y-offset -0.12]
         [label "Producers"]
         [shape (draw-piece (* pi 1/2) pi "pink")]))
  (define filters
    (new block%
         [label-scale 0.2]
         [label-x-offset 0.16]
         [label-y-offset -0.12]
         [label "Filters"]
         [shape (draw-piece 0 (* pi 1/2) "yellow")]))
  (define playlists
    (new block%
         [label-scale 0.2]
         [label-x-offset -0.16]
         [label-y-offset 0.12]
         [label "Playlists"]
         [shape (draw-piece pi (* pi 3/2) "orange")]))
  (define multitracks
    (new block%
         [label-scale 0.2]
         [label-x-offset 0.16]
         [label-y-offset 0.12]
         [label "Multitracks"]
         [shape (draw-piece (* pi 3/2) (* pi 2) "Violet")]))
  (define empty
    (blank 280 280))
  (scale
   (cc-superimpose
    (if p (send prod draw 280 280) empty)
    (if f (send filters draw 280 280) empty)
    (if pl (send playlists draw 280 280) empty)
    (if m (send multitracks draw 280 280) empty))
   1.5))

(define time-arrow
  (let* ([x (blank 0)]
         [y (blank 0)]
         [z (hc-append x (blank 800) y)])
    (pin-arrow-line 15 z
                    x cc-find
                    y cc-find
                    #:label (t "Time")
