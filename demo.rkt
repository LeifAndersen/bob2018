...
(define (mk-demo video [width 854] [height 480])
  (interactive (blank width height)
                (λ (frame)
                  (send frame show #t)
...
(define bbb-clip
  (video:playlist (video:clip "res/bbb/part01.mp4")
                  (video:clip "res/bbb/part02.mp4")
                  (video:clip "res/bbb/part03.mp4")
                  (video:clip "res/bbb/part04.mp4")))

(define bbb-clip2
  (video:playlist (video:clip "res/bbb/part02.mp4")
                  (video:clip "res/bbb/part03.mp4")
                  (video:clip "res/bbb/part04.mp4")))

(define (make-filter-slide [sepia? #f])
  (define width 640)
  (define height 480)
  (define start-time (current-seconds))
  (define (toggle-sepia sepia?)
    (void))
  #|
    (define pos (send vps get-position))
    (send vps stop)
    (send vps set-video
          (video:playlist (if sepia?
                              (video:attach-filter bbb-clip (video:sepia-filter))
                              bbb-clip)
                          #:properties (hash "start" (- (current-seconds) start-time)
                                             "end" 120)))
    (send vps render-audio #f)
    (send vps play))
|#
  (define vps (new video-player-server%
                   [video (if sepia?
                              (video:attach-filter bbb-clip2 (video:sepia-filter))
                              bbb-clip)]))
  (slide
   (code (attach-filter bunny-clip (sepia-filter)))
   (interactive (blank width height)
                (λ (frame)
                  (send frame show #t)
                  (define screen (new video-canvas%
